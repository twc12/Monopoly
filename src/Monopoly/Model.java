...
public class Model extends Observable implements Serializable {
	 ...
	 * passed all the way down from the view into this models constructor and then
	 * changes this theme variable. Right now for player icons, alex needs it hard
	 * coded BUT you should be able the change it later. just make sure that the
	 * player icons pngs are named the same as the ones in the playerIconsToPickFrom
	 * variable with "PlayerIcon.png" added to the end
	 */
	...
	/**
	 * This function accepts the game settings after the user presses "Start Game"
	 * in the start menu
	 * 
	 * @param initGameSettingsObj (GameSettings): the game settings the user
	 *                            potentially changed in the start menu
	 */
	...
	/**
	 * post_init(): After the (potentially custom) game settings are received from
	 * the view when the user presses "Start game" in the start menu then this
	 * function will run which will initialize the game creating the correct number
	 * of players
	 */
	private void post_init() {
		...
		if (totalAiPlayers > 0)
			this.notifyViewAILogsEnabled();

		theme = gameSettings.getActiveThemeString();

		List<String> playerIconsToPickFrom = new ArrayList<>();
		playerIconsToPickFrom.add("boat");
		playerIconsToPickFrom.add("car");
		playerIconsToPickFrom.add("cop");
		playerIconsToPickFrom.add("dog");
		playerIconsToPickFrom.add("evil");
		playerIconsToPickFrom.add("thimble");
		playerIconsToPickFrom.add("train");
		playerIconsToPickFrom.add("wheeler");
		...
		for (int i = 0; i < totalAiPlayers; i++) {
			players.add(new AIPlayer(totalHumanPlayers + i + 1, playerIconsToPickFrom.get(totalHumanPlayers + i), theme,
					this));
		}
	...
	/**
	 * setCurrentPlayerToNext(): This function will be called when the controller
	 * handles the end of a turn. I think for a end of turn it JUST means that we
	 * move the currentPlayer to the next one in the list and tell the view to
	 * change the player card shown in the bottom left
	 */
	...
	/**
	 * notifyViewOfDiceResult(dice1Result, dice2Result): This function will tell the
	 * view object the results of the players dice roll so it can animate that on
	 * the view and show the viewer
	 * 
	...
	/**
	 * notifyViewOfPlayerMoved(currPlayer, spaceTheyMovedTo): This function will
	 * tell the view that the player moved. It will tell them how many spaces they
	 * moved so that the view can animate the player moving.
	 * 
	...
	/**
	 * notifyViewOfNextPlayersTurn(theNextPlayer): This function will tell the view
	 * class that its the next players turn so it can pull down the next players
	 * into into the bottom left player card spot
	 * 
	...
	/**
	 * notifyViewPurchasePrompt(currPlayer, propertyObj): This function will tell
	 * the view class that a property could be purchased and it will notify the
	 * player if they want to buy it
	 * 
	...
	/**
	 * notifyViewCardDrawn(currPlayer, card): This function will tell the view class
	 * that a card was drawn so it should display the card drawn
	 * 
	 * @param currPlayer (Player): The current player whos turn it is and will have
	 *                   the cards action acted on (the view doesnt do the "acting
	 *                   on")
	...
	/**
	 * Generic notify view method for displaying string message to the view (ex:
	 * "Player X charged rent!")
	 * 
	 * @param message
	 */
	...
	public void notifyViewBankruptcy(Player player, int ammtPayed, int buildingsSoldCount,
			List<Property> propertiesSold, boolean gameOver) {
		BankruptcyMessage bankruptcyMessage = new BankruptcyMessage(player, ammtPayed, buildingsSoldCount,
				propertiesSold, gameOver);
		this.setChanged();
	 ...
	 * attempted to get out of jail. This function is only called in the view if the
	 * player.isInJail() return true so that means they MUST be in the
	 * jail.playerAttemptsToGetOutMapping. If not that is major problem, send debug
	 * message hopefully they see
	 * 
	 * @param playerInJail (Player): The player that should be in jail
	 * @return int: the number of times the player has tried to roll doubles to get
	 *         out of jail
	 */
	...
	/**
	 * putPlayerInJail(player): This function will be called when the player should
	 * be put into jail, this method should be called for when a player rolls 3
	 * doubles in a row, or pulls a card that says go to jail, or lands on the Go To
	 * Jail space
	 * 
	...
	/**
	 * For moving player to jail, using a Card to display and delay the view to fix
	 * a bug.
	 */
