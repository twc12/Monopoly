...
public class View extends Application implements Observer {
	...
	/**
	 * This is the height of the bottom controll bar section Player info card - Dice
	 * ROll Section - Button Section - Other player section
	 */
	...
	private Group rightSidePlayerPickerGroup; // This holds the VBox of Player clickable rectangles to populate the
												// center with their player card

	private final Color defaultSpaceColor = Color.WHITE;
	/**
	 * This stackPane will be holding a grid pane for dice results, hopefully In the
	 * future I can make the dice roll around in this pane
	 */
	private StackPane diceRollStackPane;

	/**
	 * This stackPane will be holding a optional additional player card info from
	 * another player in the bottom right
	 */
	...
	/**
	 * This group will hold the player cards in the bottom left If you are to change
	 * the player card then you should CLEAR THIS GROUPS CHILDREN and then add the
	 * new player card
	 */
	...
	/**
	 * The RealEstate property selected in the player info panel for building. Null
	 * = none selected.
	 */
	...
	/**
	 * This is a list of Stack Panes. The stack panes are all of the spaces on the
	 * outskirts of the board. Every space should be built with a stack pane. That
	 * way when I move a player I can just get the x and y location of that pane and
	 * set the visual object for that player to that x and y
	 */
	private List<StackPane> listOfSpacesPanes;
	/**
	 * I was struggling figuring out how to figure out where the current player is
	 * at regarding the stack pane visual type of board. I needed to know where they
	 * are at so I can start at the correct spot in the `listOfSpacesPanes` above.
	 * 
	 * The goal here is for 1. the view to receive a PlayerMovedMessage 2. Then we
	 * figure out where that player is on the board using this map 3. Then we find
	 * the index of that pane in the listOfSpacesPanes 4. Then we loop `while
	 * (currAmmtMoved < ammtToMove)` to move the player one space at a time using
	 * the Screen X Y values of the StackPane pulled from `listOfSpacesPanes`
	 */
	private Map<Player, StackPane> whichStackPanesPlayersAreOn;

	/**
	 * This holds a mapping of player objects to the visual player pieces This is
	 * used for moving, so when the view receives a PlayerMovedMessage we can get
	 * the object that should be translated for every move.
	 */
	private Map<Player, Circle> playerObjToPlayerPiece;

	/**
	 * I needed a way position the circles on each space so that they all moved out
	 * of their way at least a little bit when they are all on the same space.
	 * 
	 * These lists will hold a translate amount of each circle for when there are
	 * multiple per space. The first detected circle will be moved to xDeltas[0],
	 * yDeltas[0] and so on.
	 */
	...
	/**
	 * I need to store a second set of circle objects for the right player picker
	 * side So I dont have to load thenm new each and every time
	 */
	private Map<Player, Circle> rightSideIconsMap;

	/**
	 * This VBox will have messages added to it for every ai decision made. It will
	 * get populated in the `update()` function when we recieve a
	 * `aiDecisionMessage` from the model
	 */
	private VBox aiLoggerVBox;

	/**
	 * This Customizes the look of the ai logger text labels Hacker style text
	 */
	...
	/**
	 * This is used for displaying a properties card detailed information when a
	 * user clicks on it
	 */
	...
	/**
	 * This is used for moving the player to jail after they are sent to jail, it
	 * will hold the stack pane that jail is made out of
	 */
	...
	public void start(Stage stage) throws Exception {
		...
		resultsOfInputVBox.setPadding(new Insets(10));
		...
		resultsOfInputVBox.getChildren().addAll(allowSkipPropPurchLabel, passingGoAmountLabel, propPricesMultiLabel,
				freeParkCollectsFundsLabel, amtOfStartingMoneyLabel, tradingWithOthersLabel, numOfHumanPlayersLabel,
				numOfAiPlayersLabel);
		...
		pricePerSpaceMultiplyerInput.getItems().addAll("1 (default)", "2x (Meh)", "3x (Woozers)", "5x (Good luck)",
				"10 (Ultra Mega Extreme)");
		...
		// NUMBER OF PLAYERS
		ChoiceBox<Integer> numberOfAiPlayersChoiceBox = new ChoiceBox<>();
		; // Message to choice box below "IT WILL BE DEFINED LATER TRUST"
		...
		numberOfHumanPlayersChoiceBox.setOnAction(event -> {
			...
			if (numOfHumans == null) // this is a fix to when I would change a value in the ai box it would activate
										// this box when i didnt want to
				return;
			if (numOfHumans == 0) {
				boolean numOfAisIsNullOrLessThan2 = numberOfAiPlayersChoiceBox.getValue() == null
						|| numberOfAiPlayersChoiceBox.getValue() < 2;
			...
			boolean numOfAisIsNullOrZero = numberOfAiPlayersChoiceBox.getValue() == null
					|| numberOfAiPlayersChoiceBox.getValue() == 0;
			if (numOfHumans == 1 && numOfAisIsNullOrZero) { // only forcefully update the other box if it hasnt been
															// picked or is set to zero
			...
			// Allow the user to select the number of ai players that would fill at most the
			// 8 total allowed, only show options for that
		...
		numberOfAiPlayersChoiceBox.setOnAction(event -> {
			...
			if (numOfAis == null) // this is a fix to when I would change a value in the human box it would
									// activate this box when i didnt want to
				return;
			// if the user selects zero ais, then the other box must be 2 or more
			if (numOfAis == 0) {
				boolean numOfHumansIsNullOrLessThan2 = numberOfHumanPlayersChoiceBox.getValue() == null
						|| numberOfHumanPlayersChoiceBox.getValue() < 2;
			...
			boolean numOfHumansIsNullOrZero = numberOfHumanPlayersChoiceBox.getValue() == null
					|| numberOfHumanPlayersChoiceBox.getValue() == 0;
			if (numOfAis == 1 && numOfHumansIsNullOrZero) { // only forcefully update the other box if its unchosesn or
															// is zero
			...
			// Now update the human player options to only have the rest over amount to hold
			// the max of 8 total true
		...
		// Create a load button that will prompt the user to select a save file that
		// ends in .monopoly
		...
		loadGameButton.setOnAction(event -> {
			...
			FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Monopoly Save File(*.monopoly)",
					"*.monopoly");
			...
			// if the user selected a file, then load the game based on this model and
			// start!
		...
		inputsVBox.setPadding(new Insets(10));
		inputsVBox.getChildren().addAll(allowSkippingPropertyPurchaseButton, moneyPassingGoInput,
				pricePerSpaceMultiplyerInput, freeParkingRewardsButton, startingMoneyInput, tradingButton,
				humanPlayersChoicesHBox, aiPlayersChoicesHBox, startAndLoadButtonsHBox);
		...
		HBox themeChoicesHBox = new HBox(15);
		themeChoicesHBox.setPadding(new Insets(10));
		ToggleGroup toggleGroup = new ToggleGroup();

		// --- BASE MONOPOLY
		VBox baseMonopolyThemeChoiceVBox = new VBox(10);
		baseMonopolyThemeChoiceVBox.setAlignment(Pos.CENTER);
		...
		baseMonopolyPreviewView.setPreserveRatio(true);
		baseMonopolyPreviewView.setFitWidth(MAX_IMAGE_WIDTH);
		...
		// --- PIRATES
		VBox pirateThemeChoiceVBox = new VBox(10);
		pirateThemeChoiceVBox.setAlignment(Pos.CENTER);
		...
		piratePreviewView.setPreserveRatio(true);
		piratePreviewView.setFitWidth(MAX_IMAGE_WIDTH);
		...
		// --- TUCSON
		VBox tucsonThemeChoiceVBox = new VBox(10);
		tucsonThemeChoiceVBox.setAlignment(Pos.CENTER);
		...
		tucsonPreviewView.setPreserveRatio(true);
		tucsonPreviewView.setFitWidth(MAX_IMAGE_WIDTH);
		...
		themeChoicesHBox.getChildren().addAll(baseMonopolyThemeChoiceVBox, pirateThemeChoiceVBox,
				tucsonThemeChoiceVBox);
	 ...
	 * pressed from the start menu. The reason for this separation is because the
	 * code in here relies on knowing how many players, and that information isnt
	 * known until the player inputs that data. So only once the player presses
	 * "Start game" does the controller->model get those new game settings and that
	...
	private Scene createMainGame() {
		...
		// Put all the players pieces on their corresponding space, then track which
		// space the players are on
		List<Player> allPlayers = controller.getAllPlayers();

		// for each player, pull their space, then find the stack pane for their current
		// space
		for (Player currPlayer : allPlayers) {

			// pull out the space, then find the stack pane that has that space in the user
			// data
		...
		ParallelTransition combine;
		
		Image tumbleImage = null;
		ImageView tumbleImageView = null;
	
		
//		tumbleImageView.setPreserveRatio(true);
		
		if (theme.equals("tucsonTheme")) {
			tumbleImage = new Image("/" + theme + "/tumbleweed.png");
			tumbleImageView = new ImageView(tumbleImage);
			tumbleImageView.setFitWidth(300);
			tumbleImageView.setPreserveRatio(true);
			tumbleImageView.setOpacity(.8);
			PauseTransition pause = new PauseTransition(Duration.seconds(30));
			transition = new TranslateTransition();
			rotate = new RotateTransition();
			
			
			transition.setNode(tumbleImageView);
			transition.setDuration(Duration.seconds(8));
			
			rotate.setNode(tumbleImageView);
			rotate.setDuration(Duration.seconds(7));
	        ...
	        combine = new ParallelTransition(tumbleImageView,transition,rotate,pause);
	        combine.setDelay(Duration.seconds(15));
	        
	        combine.setCycleCount(ParallelTransition.INDEFINITE);
	        combine.setDelay(Duration.seconds(15));
	        combine.play();
		...
		if (theme.equals("tucsonTheme")) {
			...
			bottomFrameImageView.setFitWidth(1155);
			bottomFrameImageView.setTranslateY(530);
			bottomFrameImageView.setTranslateX(-45);
		}
		...
		if (theme.equals("tucsonTheme"))
			root.getChildren().add(tumbleImageView);
		...
		Label saveButtonLabel = new Label("Save Game");
		saveButtonLabel.setFont(new Font(15));
		saveButtonLabel.setTextFill(Color.WHITE);
		...
		saveButtonStackPane.setOnMouseClicked(event -> {
			...
			FileChooser.ExtensionFilter extFilter = new FileChooser.ExtensionFilter("Monopoly Save Files(.monopoly)",
					".monopoly");
			fileChooser.getExtensionFilters().add(extFilter);
		...
		return gameScene; // THIS RETURNS TO `def start() -> StartGameBtn.onAction()` code look up ^^ a
							// function
	}

	/**
	 * buildRightPlayerPicker(): This function will create the right side rectangles
	 * for selecting a player to see their properties and information Instead of
	 * returning it overwrites the VBox that is in the right side group so it
	 * displays the current "other players" meaning it WONT show the `currPlayer`
	 * rectangle
	 * 
	 * @param currPlayer (Player): The current player in the game to not show their
	 *                   box
	 */
	private void buildRightPlayerPicker(Player currPlayer) {
		...
		// Loading the images each time would make the game slow, so now we load them to
		// a map of players to circles
		...
		for (Player player : allPlayers) {
			...
			// if its not he current player then make the fully feature rich rectangle with
			// color and rectangle
			else {
	...
	private void showOtherPlayersInfoInBottomRight(Player selectedOtherPlayer) {
		...
		if (selectedOtherPlayer.equals(previousSelectedOtherPlayer)) {
			...
			if (otherPlayerInfoCardStackPane.getChildren().size() > 0
					&& !(otherPlayerInfoCardStackPane.getChildren().getFirst() instanceof ScrollPane)) { // clear
																											// playerinfocard,
																											// keep ai
																											// console
				otherPlayerInfoCardStackPane.getChildren().removeLast();
	...
	public StackPane buildMonopolyBoard() {
		...
		this.infoToTellPlayer = infoToTellPlayer; // we store it so in the future we can change the text to inform the
													// user of something
	...
	private void addPropertySpaceObjToBoard(GridPane mainBoardGridPane, Space space, int col, int row, int rotateAmmt,
		...
		// FUTURE PROOOF CODE: this code should stay after that function above (line
		// 448) is made because we need clickable stuff
		// Whenever this -- on the board -- property pane is panned over show the
		// detailed stats
	...
	private StackPane buildBottomSection() {
		...
		// Player info group - This group is used to change the property card that is
		// held in the bottom left
		...
		diceRollStackPane.setStyle("-fx-border-color: " + uiDividerColor
				+ "; -fx-border-width: 3; -fx-padding: 5; -fx-background-color: " + uiColorString + " ;");
...
//		otherPlayerInfoCardStackPane.setStyle("-fx-border-color: "+uiDividerColor + "; -fx-border-width: 3; -fx-padding: 5; -fx-background-color: "+ uiColorString + " ;");
	...
	private Group buildMainButtons() {
		// Buttons
		// FUTURE NOTE - The get out of jail options will replace the children of this
		// group with its own buttons
		// - Then after its done it will replace it back with the object that is stored
		// in the `coreButtonsVBox` attribute
		...
		coreButtonsVBox.setStyle("-fx-border-color: " + uiDividerColor
				+ "; -fx-border-width: 3; -fx-padding: 5; -fx-background-color: " + uiColorString + " ;");
		coreButtonsVBox.setPrefHeight(bottomHBoxHeight + 5);
		coreButtonsVBox.setAlignment(Pos.CENTER);
		this.coreButtonsVBox = coreButtonsVBox; // I need this saved so the getOutOfJailLogic can bring these buttons
												// back
	...
	/**
	 * createVisualPlayerInfoCard(currPlayer): This function will create a visual
	 * player card, showing amount of money, player name a scroll pane of
	 * properties. This visual card is usually placed in the bottom left of the
	 * screen to show the current players info
	 * 
	 * @param currPlayer (Player): The new player that will have their info shown
	 * 
	 * @return Node: The Java fx pane that will be a visual representation of a
	 *         player card
	 */
	private Node createVisualPlayerInfoCard(Player currPlayer) {
		...
		visualPlayerCardGridPane.setStyle("-fx-border-color: " + uiDividerColor
				+ "; -fx-border-width: 3; -fx-padding: 5; -fx-background-color: " + uiColorString + " ;");
		...
		Label playerGetOutOfJailCardsAmtLabel = new Label(
				"Ammount of Get out of jail cards: " + currPlayer.getAmmtOfGOOJCards());
	 ...
	 * much rent will they get.
	 * 
	 * @param player (Player): The player we are basing the owned properties of
	 * @return ScrollPane: A horizontal scroll pane showing the properies of the
	 *         player
	 */
	private ScrollPane createScrollPaneOfPlayersProperties(Player player) {
		...
		// This sets the default height of a EMPTY properties scroll pane to be the same
		// as if there were properties so there isnt a suddon jolt when a property is
		// purchased
		propertiesGridPane.setMinHeight(widthOfPlayerCardProperties * 1.6 + paddingAmmtAroundProperties); // the 1.6 is
																											// to
																											// convert
																											// width to
																											// height
		...
		// go over every property they have, create a visual stack pane for it, add it
		// to the grid pane
		for (Property currProperty : playersProperties) {
			...
			// check if realestate, building the 5 "build-progress-dots" on top of a real
			// estate card and add build-button onclick logic
			if (currProperty instanceof RealEstate re) {
				...
				// adding on-click selects/deselects for building; enables/disables the Build
				// button
				visualPropertyInfoCard.setOnMouseClicked((e) -> {
					selectedPropertyToBuild = re;
					if (!re.getIfCanBuild() || re.getBuildingStage() >= 5
							|| !(re.getOwner().equals(controller.getCurrentPlayer()))) { // if already fully developed,
																							// don't enable the build
																							// button when i click a
																							// card
					...
					// if another player clicked on this card from someone elses info card then
					// allow trading
				...
				// if not realestate, will only add an on-click for details and not build any
				// dots
			} else {
				visualPropertyInfoCard.setOnMouseClicked((e) -> {
		...
		scrollablePropertiesPane.setMinHeight(widthOfPlayerCardProperties * 1.6 + 25); // the 1.6 is from how the cards
																						// heigh is determined in
																						// `buildSpaceCard()`
	...
	/**
	 * When the user presses "end turn" then let the controller know to move to the
	 * next player
	 */
	...
	private void handleTradeButton() {
		...
		backgroundRect.setStrokeWidth(5);
		...
		// moved them before the end so that we can conditionally add the next two
		// options
		...
		// The amount of get out of jail cards, if the user has them then allow them to
		// offer them
		...
		if (buyingPlayer.getListOfProperties().size() != 0) {
			...
			for (Property buyersProperty : buyingPlayer.getListOfProperties()) {
				...
				propertyCheckBox.setOnAction(event -> {
					// if the box is now selected then add the property to the list of properties to
					// offer
					if (propertyCheckBox.isSelected() == true) {
						...
						// we have to store this label in a hash map so we know which vbox child to
						// remove when deselected
						propertiesToLabels.put(buyersProperty,
								new Label(buyersProperty.getName() + " worth $" + buyersProperty.getPurchaseAmount()));
						playersTradeOutputsVbox.getChildren().add(propertiesToLabels.get(buyersProperty)); // add label
																											// to
																											// outputs
																											// vbox
					} else {
						listOfPropertiesToOffer.remove((Property) propertyCheckBox.getUserData()); // remove the
																									// property when
																									// deselected
		...
		acceptBtn.setOnAction(event -> {
			...
			controller.executeTrade(buyingPlayer, sellingPlayer, potentialTradeProperty, listOfPropertiesToOffer,
					ammountOfMoneyToPay, ammountOfGOOJCards);
			...
			showOtherPlayersInfoInBottomRight(sellingPlayer); // reusing this function, it will "refresh" the other
																// players' panel correctly dynamically
			if (otherPlayerInfoCardStackPane.getChildren().size() == 1) {// if it refreshes to the ai console only,
																			// redraw on top
	...
	private void animateDiceRoll(int dice1Result, int dice2Result) {
		...
		BorderStroke borderStroke = new BorderStroke(Color.TRANSPARENT, BorderStrokeStyle.SOLID, CornerRadii.EMPTY,
				new BorderWidths(2));
	...
	/**
	 * createADice(diceAmmt): This function will create a dice looking square using
	 * a grid pane. It will have black circles in the grid and there will be
	 * `diceAmmt` of circles
	 * 
	 * @param diceAmmt (int): The amount of dice requested to show on the dice
	 * @return GridPane: A 5x5 with the number of black dots being the `diceAmmt`
	 *         given
	 */
	private GridPane createADice(int diceAmmt) {
		...
		// Record which slots on the dice got filled, so we can fill others with empty
		// space
		Circle newDot;
		switch (diceAmmt) {
		case 1:
	...
	/**
	 * populatePlayerCardWithNewInfo(theNextPlayer): This function is called after
	 * we receive a message from the model that its the next players turn. THis
	 * function will use the player object given to replace the info card on the
	 * bottom left.
	 * 
	...
	private void showOptionsForGettingOutOfJail(Player currentPlayer) {
		...
		// if the player can still attempt to roll doubles then add a button for it, but
		// if they dont have $50 then they must roll doubles
	...
	public void update(Observable model, Object message) {
		...
		// if the message is that a player landed on an unowned property, buying is
		// optional
			...
			Timeline promptDelay = new Timeline(new KeyFrame(Duration.seconds(2), e -> {
		...
		// if the message is that a chance/chest card was drawn, show the card
			...
			Timeline cardDelay = new Timeline(new KeyFrame(Duration.seconds(2), e -> {
		...
		// if the message is that a Ai Took an action, display the action to the other
		// players
		else if (message instanceof AiActionMessage) {
			// IMPORTANT NOTE; We must pause between messages some how because the ai will
			// kinda happen instantly, this is a problem.
...
//			aiLoggerVBox.getChildren().add(newAiActionLabel);		
			Timeline delay = new Timeline(new KeyFrame(Duration.seconds(6), e -> {
				Label newAiActionLabel = new Label(aiActionMsg.getAiAction());
...
//	        controller.processEndTurn();
	...
	public void showPurchasePrompt(Player player, Property property) {
		...
		hboxContainer.setStyle("-fx-border-width: 3;" + "-fx-padding: 20;");
	 ...
	 * larger version of the detailed property info which floats on the board until
	 * pressed out
	 * 
	...
	public StackPane buildSpaceCard(Property testSpace, double cardWidth) {
		...
		if (testSpace instanceof RealEstate) {
			...
			// Changed from this to the new line, this was causing cards to expend
			// horizontally to the window edges
			// colorRect.widthProperty().bind(card.widthProperty().subtract(innerWidthOffset*.8));
			// // Works for rest of team
			colorRect.setWidth(cardWidth - innerWidthOffset * 0.8); // Works for jarrod, jarrod should be only one using
																	// this line of code
																	// since the other implementation for some
																	// unknowable reason doesn't work
																	// on his computer
...
//	        bodyFlow.setTextAlignment(TextAlignment.CENTER);
			bodyFlow.setMaxWidth(cardWidth - innerWidthOffset - 5); // CHANGED TO -5 so text wouldnt wrap in properties
																	// owned lists in player info card
	...
	private ScrollPane buildAILoggerScrollPane() {
		...
		aiLoggerScrollPane.setStyle("-fx-background-color: black; -fx-background: black; " + "-fx-border-color: "
				+ uiDividerColor + "; -fx-border-width: 3; -fx-padding: 5;");
